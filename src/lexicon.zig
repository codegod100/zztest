...
pub fn Query(comptime nsid: []const u8) type {
    return struct {
        pub fn run() !std.json.ObjectMap {
            var map = try enumTable(fba_alloc);
            defer map.deinit();
            ...
            const profile_string = @embedFile("./getProfile.json");
            if (nsid_enum) |e| {
                switch (e) {
                    NSID.getProfile => {
                        profile = try parseJson(profile_string);
                    },
                }
            }
            var buffer = std.ArrayList(u8).init(fba_alloc);
            try std.json.stringify(profile, .{}, buffer.writer());
            ...
            const def_string = @embedFile("./defs.json");
            comptime {
                const defs = try parseJson(def_string);
...
//     return buffer.items;
// }
fn parseJson(comptime content: []const u8) !std.json.Value {
    var fbs = comptime std.io.fixedBufferStream(content);
    var reader = std.json.reader(fba_alloc, fbs.reader());
    defer reader.deinit();

    const json_struct = try std.json.parseFromTokenSourceLeaky(std.json.Value, fba.allocator(), &reader, .{});
    // defer json_struct
    return json_struct;
}
